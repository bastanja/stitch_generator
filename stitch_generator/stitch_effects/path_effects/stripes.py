...
def stripes(steps: Array1D, sampling_function: SamplingFunction, step_ratio: float = 0.1) -> StitchEffect:
    return lambda path: stripes_along(path, steps=steps, sampling_function=sampling_function, step_ratio=step_ratio)


def simple_stripes(repetitions: int, sampling_function: SamplingFunction, step_ratio: float = 0.1) -> StitchEffect:
    return lambda path: stripes_along(path, steps=sample_by_number(repetitions), sampling_function=sampling_function,
                                      step_ratio=step_ratio)


def stripes_along(path: Path, steps: Array1D, sampling_function: SamplingFunction, step_ratio: float):
    return stripes_between(*get_boundaries(path), steps=steps, sampling_function=sampling_function,
                           length=path.length, step_ratio=step_ratio, circular=path.is_circular)


def stripes_between(boundary_left, boundary_right, steps: Array1D, sampling_function: SamplingFunction, length: float,
