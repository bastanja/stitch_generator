...
from stitch_generator.stitch_effects.utilities.variable_running_stitch import variable_underlay_along
...
def underlay_dense(inset: float, stitch_length: float, spacing: float) -> StitchEffect:
    def underlay(path: Path) -> Array2D:
        ...
        fill = variable_underlay_along(path=path, stroke_spacing=spacing, stitch_length=stitch_length)
