...
def get_tatami():
    return alternating_tatami_sampling(stitch_length=4, include_endpoint=True,
                                       offsets=(0, 1 / 3, 2 / 3),
                                       alignment=0.5,
...
def stitch_effects():
    ...
    yield partial(meander_along, stitch_spacing=1, connect_function=combine_start_end(
        line_with_sampling_function(get_tatami())), length=100)
    ...
    yield partial(repeat_along, repetitions=9, sampling_function=get_tatami(), length=100, step_ratio=0.1)
