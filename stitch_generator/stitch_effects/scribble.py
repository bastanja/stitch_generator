...
def scribble(repetitions: int, stitch_length: float, noise_scale: float = 1, noise_offset: float = 0) -> StitchEffect:
    return lambda path: scribble_along(path, repetitions=repetitions,
                                       sampling_function=tatami_sampling(stitch_length, include_endpoint=False,
                                                                         offsets=[0, 1 / 3, 2 / 3], alignment=0.5),
