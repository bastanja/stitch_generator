...
def simple_rounded_rect(
    width: float, height: float, corner_radius
) -> Tuple[Function2D, Function2D]:
    return rounded_rect(width, height, [(corner_radius, corner_radius)] * 4)


def rounded_rect_with_corner_radii(
    width: float, height: float, corner_radii: Sequence[float]
) -> Tuple[Function2D, Function2D]:
...
def _shape_parts(width: float, height: float, corner_radii):
    ...
    corners = [
        d(multiply_functions(c, constant_direction(x, y)))
        for d, c, x, y in zip(corner_forward_backward, corners, scale_x, scale_y)
    ]

    # connecting line functions
    connection_points = [
        (a(1), b(0)) for a, b in zip([corners[-1]] + corners[:-1], corners)
    ]
...
def _direction_parts(corner_radii):
    corner_directions = [
        (repeat(0.25, ellipse_direction(rx=x, ry=y), mode="wrap"))
        for x, y in corner_radii
    ]
    corner_directions = [shift(i, f) for i, f in zip(range(4), corner_directions)]
    connection_directions = [
        constant_direction(x, y) for x, y in ((1, 0), (0, 1), (-1, 0), (0, -1))
    ]
    directions = list(
        itertools.chain.from_iterable(zip(connection_directions, corner_directions))
    )
    return directions
