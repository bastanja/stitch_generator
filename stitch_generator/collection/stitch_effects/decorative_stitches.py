...
def decorative_stitches_collection(
    spacing: float = 2.5, stitch_width: float = 5, pattern_spacing: float = 20
):
    yield alternating_triangles(
        spacing=spacing, line_length=4, width=stitch_width, repetitions=3
    )
    yield arrow_chain(arrow_width=stitch_width, arrow_length=2, arrow_spacing=spacing)
    yield chevron_stitch(
        spacing=spacing * 2, line_length=3, width=stitch_width, repetitions=5
    )
    yield cretan_stitch(
        spacing=spacing * 2,
        stitch_width=0.1,
    ...
    yield cretan_stitch(
        spacing=spacing * 2, stitch_width=0.1, stitch_length=3.5, repetitions=4
    )
    yield e_stitch(spacing=spacing, line_length=4, stitch_length=10, angle=45)
    yield feather_stitch(
        spacing=spacing, stitch_width=0, stitch_length=3.5, repetitions=2
    )
    ...
    yield stem_stitch(
        spacing=spacing, stitch_width=0.6, stitch_length=5, repetitions=5, angle=-25
    )
    yield stem_stitch(
        spacing=spacing,
    ...
    yield three_arrows(
        arrow_spacing=2,
        group_spacing=pattern_spacing,
        start_end_spacing=pattern_spacing / 2,
        stitch_length=3,
    )
    yield x_motif_stitch(spacing=spacing, width=stitch_width, length=4)


def alternating_triangles(
    spacing: float, line_length: float, width: float, repetitions: int
):
    """
...
def arrow_chain(
    arrow_width: float, arrow_length: float, arrow_spacing: float
) -> StitchEffect:
    """
    ...
    """
    single_arrow = np.array(
        ((-arrow_width / 2, -arrow_length), (0, 0), (arrow_width / 2, -arrow_length))
    )
...
def cretan_stitch(
    ...
    """
    motif = zigzag_motif(stitch_width, stitch_length, repetitions) + (
        zigzag_width / 2,
        0,
    )
    motif_gen = repeat_motif_mirrored(motif)
...
def e_stitch(
    spacing: float, line_length: float, stitch_length: float, angle: float
) -> StitchEffect:
    """
    ...
    """
    motif = line_shape((0, 0), (line_length, 0))(
        subdivide_by_length(line_length, stitch_length)
    )
    ...
    return motif_to_points(
        motif_placement=subdivision_by_length(spacing),
...
def feather_stitch(
    spacing: float, stitch_width: float, stitch_length: float, repetitions: int
):
    """
    ...
    Args:
        ...
        repetitions: How often a line pointing to the side is repeated.
    """
    motif = rotate_by_degrees(
        zigzag_motif(stitch_width, stitch_length, repetitions), 45
    ) + (1, 0)
    motif_gen = repeat_motif_mirrored(motif)
    subdivision = subdivision_by_length(spacing)
    return motif_chain(subdivision, motif_gen, constant(0))
...
def overlock_stitch(length: float, width: float):
    ...
    back_stitch_subdivision = add_start(
        add_end(subdivision_by_length_with_offset(segment_length=length, offset=0.5))
    )

    def effect(path):
        constant_width_path = Path(
            path.shape, path.direction, constant(width), constant(0.5)
        )
        ...
        return np.concatenate(
            (stitches, right(1), left(1), back_stitches, back_stitches[-2::-1])
        )
...
def three_arrows(
    ...
    combined = np.concatenate(
        (
            single_arrow,
            single_arrow + (arrow_spacing, 0),
            single_arrow + (arrow_spacing * 2, 0),
        )
    )
    motif_subdivision = free_start(
        start_end_spacing, free_end(start_end_spacing, regular(group_spacing))
    )
    return motif_to_segments(
        motif_subdivision,
        regular(stitch_length),
        repeat_motif(combined),
        motif_length=3,
    )
