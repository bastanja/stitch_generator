...
def satin_ellipse(
    ...
    underlay_inset: float = 0.5,
    underlay_spacing: float = 1.5,
    satin_spacing: float = 0.2,
    return_to_start: bool = False,
):
    # create stitch effects
    underlay_effect = double_zigzag(spacing_function=regular(underlay_spacing))
    zigzag_effect = zigzag(
        spacing_function=subdivision_by_length_with_offset(
            segment_length=satin_spacing, offset=0.5
        )
    )

    return _satin_ellipse(
        width=width,
        height=height,
        stitch_length=stitch_length,
        pull_compensation=pull_compensation,
        underlay_inset=underlay_inset,
        return_to_start=return_to_start,
        underlay_effect=underlay_effect,
        top_effect=zigzag_effect,
    )


def satin_circle(
    diameter: float,
    stitch_length: float = 3,
    pull_compensation: float = 0,
    underlay_inset: float = 0.5,
    underlay_spacing: float = 1.5,
    satin_spacing: float = 0.2,
...
def tatami_ellipse(
    ...
    # create stitch effects
    underlay_effect = _underlay_effect(
        stitch_length=stitch_length, line_spacing=underlay_spacing
    )
    line_subdivision = add_start(
        add_end(alternate_direction(tatami_3_1(segment_length=5)))
    )
    satin_effect = meander(
        spacing_function=subdivision_by_length_with_offset(
            segment_length=satin_spacing, offset=0.5
        ),
...
def ellipse_path(width: float, height: float):
    ...
    return Path(
        shape=shape,
        direction=direction,
        width=scale(width, half_circle),
        stroke_alignment=constant(0.5),
    )


def _paths(
    width: float, height: float, underlay_inset: float, pull_compensation: float
) -> Tuple[Path, Path]:
    inner_path = ellipse_path(
        width=height - underlay_inset, height=width - underlay_inset
    )
...
def _underlay_effect(stitch_length, line_spacing):
    return underlay_contour_zigzag(
        inset=0, stitch_length=stitch_length, spacing=line_spacing
    )
