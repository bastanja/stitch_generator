...
from stitch_generator.shapes import circle_shape, line_shape
...
from stitch_generator.subdivision import remove_end, subdivision_by_number


def zigzag_rectangle(
    width: float, height: float, repetitions: int, horizontal: bool, flip: bool = False
):
    if horizontal:
        return _zigzag_rectangle(
            width=width, height=height, repetitions=repetitions, flip=flip
        )
    else:
        return rotate_90(
            _zigzag_rectangle(
                width=height, height=width, repetitions=repetitions, flip=flip
            )
        )


def _zigzag_rectangle(
    width: float, height: float, repetitions: int, flip: bool = False
):
    half_width = width / 2
...
def line_motif(length: float, repetitions: int):
    motif = zigzag_rectangle(
        width=0.1, height=length, repetitions=repetitions, horizontal=True
    )
...
def star_motif(num_spikes: int, inner_radius: float, outer_radius: float):
    ...
    result = zigzag_between(
        outer,
        inner,
        spacing_function=remove_end(subdivision_by_number((num_spikes * 2))),
        length=1,
    )
