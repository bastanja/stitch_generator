...
def subdivision_with_wave_offset(
    ...
    return subdivision_with_varying_offset(
...
def subdivision_with_arc_offset(
    segment_length: float, steps: int, function_range=(0, 1)
):
    return subdivision_with_varying_offset(
        segment_length=segment_length,
...
def subdivision_with_triangle_offset(
    segment_length: float, steps: int, function_range=(0, 1)
):
    offset_function = to_range(repeat(2, linear_0_1, mode="reflect"), function_range)
    return subdivision_with_varying_offset(
        segment_length=segment_length, steps=steps, offset_function=offset_function
    )


def to_range(offset_function, function_range):
    return compose(
        offset_function, linear_interpolation(function_range[0], function_range[1])
    )


def subdivision_with_varying_offset(
    segment_length: float, steps: int, offset_function: Function1D, alignment=0.5
) -> SubdivisionFunction:
    """
    ...
    subdivision = partial(
        subdivide_by_fixed_length, segment_length=segment_length, alignment=alignment
    )
