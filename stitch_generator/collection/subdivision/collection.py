...
def subdivision_functions():
    ...
    yield free_start_end(
        alternate_direction(
            subdivision_with_triangle_offset(segment_length=3.5, steps=30)
        )
    )
    yield free_start_end(
        subdivision_with_arc_offset(
            segment_length=3, steps=30, function_range=(0.75, 0.25)
        )
    )
    yield free_start_end(
        subdivision_with_wave_offset(
            segment_length=3, steps=60, function_range=(0.2, 0.8)
        )
    )
    yield free_start_end(subdivision_with_triangle_offset(segment_length=3, steps=40))
    yield free_start_end(
        subdivision_with_triangle_alignment(
            segment_length=3, steps=30, function_range=(0.35, 0.65)
        )
    )
