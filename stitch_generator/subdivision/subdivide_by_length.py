...
def subdivide_by_length(total_length: float, segment_length: float) -> Array1D:
    if (
        np.isclose(total_length, 0)
        or (segment_length > total_length)
        or np.isclose(segment_length, 0)
    ):
...
def subdivision_by_length(segment_length: float) -> SubdivisionFunction:
    def f(total_length: float):
        return subdivide_by_length(
            total_length=total_length, segment_length=segment_length
        )

    return f


def subdivide_by_length_with_offset(
    total_length: float, segment_length: float, offset: float
) -> Array1D:
    segment_to_length = segment_length / total_length if total_length > 0 else 0
    offset = (offset % 1) * segment_to_length

    values = (
        subdivide_by_length(total_length=total_length, segment_length=segment_length)[
            :-1
        ]
        + offset
    )
...
def subdivision_by_length_with_offset(
    segment_length: float, offset: float
) -> SubdivisionFunction:
    def f(total_length: float):
        return subdivide_by_length_with_offset(
            total_length=total_length, segment_length=segment_length, offset=offset
        )
