...
from stitch_generator.collection.stitch_effects.stitch_effects import (
...
test_paths = [
    ...
    Path(
        *line(origin=(0, 0), to=(100, 0)),
        width=constant(10),
        stroke_alignment=constant(0.5),
    ),
    # path with bezier shape and a width of zero at the start and end
    Path(
        *bezier(control_points=((0, 0), (50, 50), (100, 0))),
        width=compose(arc, linear_interpolation(0, 15)),
        stroke_alignment=constant(0.5),
    ),
    # circular path
...
def test_stitch_in_rectangle_bounds(effect):
    ...
    width, height = 100, 30
    path = Path(
        *line(origin=(0, height / 2), to=(width, height / 2)),
...
def test_stitch_in_circle_bounds(effect):
    ...
    radius, width = 50, 20
    path = Path(
        *circle(radius=radius), width=constant(width), stroke_alignment=constant(0.5)
    )
    ...
    stitches_inside_outer = in_circle(
        stitches, radius=radius + half_width, center=(0, 0)
    )
    assert np.all(stitches_inside_outer == True)

    # check that no stitches lie inside the inner boundary
    stitches_inside_inner = in_circle(
        stitches, radius=radius - half_width, center=(0, 0)
    )
