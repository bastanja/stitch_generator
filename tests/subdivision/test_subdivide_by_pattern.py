...
def test_pattern_from_spaces(spaces, expected_result, with_start: bool, with_end: bool):
    result = pattern_from_spaces(
        spaces=spaces, with_start=with_start, with_end=with_end
    )
...
@pytest.mark.parametrize(
    "pattern, pattern_length, total_length, alignment, offset, expected_values",
    test_values,
)
def test_subdivision_by_pattern(
    pattern, pattern_length, total_length, alignment, offset, expected_values
):
    subdivision_function = subdivision_by_pattern(
        pattern=pattern,
