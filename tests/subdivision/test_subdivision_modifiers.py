...
def test_free_start(subdivision_function):
    free_start_length = 5
    modified = free_start(
        start_length=free_start_length, subdivision_function=subdivision_function
    )
...
def test_free_end(subdivision_function):
    free_end_length = 3
    modified = free_end(
        end_length=free_end_length, subdivision_function=subdivision_function
    )
...
def test_cycle_offsets():
    partial_value_function = partial(
        subdivide_by_fixed_length, segment_length=1, alignment=0
    )
...
def test_cycle_alignments():
    partial_value_function = partial(
        subdivide_by_fixed_length, segment_length=1, offset=0
    )
